...
function calcTwoPairs(diceCount) {
  let firstpair = calcPair(diceCount);
  diceCount[firstpair / 2] -= 2;
  let secoundpair = calcPair(diceCount);
  while (secoundpair == firstpair) {
    secoundpair = calcPair(diceCount);
    diceCount[secoundpair / 2] -= 2;
    if (secoundpair == 0) {
      return 0;
    }
  }
  return firstpair + secoundpair;
}
...
function calcChance(diceCount) {
  let chance = 0;
  for (let i = minDice; i <= maxDice; i++) {
    chance += i * diceCount[i];
  }
  return chance;
}
